 ...
 */
 public class OnOrderChartWorker 
                extends SwingWorker<Integer, Integer>
 {
   ...
   public OnOrderChartWorker(final CloseableTabbedPane tabbedPane,
   ...
   {
     InvManagerMainFrame.setWorkingStatus(true, "Creating on order chart...");
     ...
     for (ProductLine prodLine : jpa.getProductLines(vendor))
     {
       ...
       for (Item i : items)
       {
         ...
         if (null != sqty && sqty.getQtyOnOrder() > 0)
         {
           newQty += sqty.getQtyOnOrder();
         }         
       }
     ...
     jfc = ChartFactory.createPieChart("Number of Inventory On Order for " 
              + store.getStoreName(),
     ...
     pp.setNoDataMessage("No Inventory On Order Data Found");
     pp.setCircular(false);
     pp.setLabelGap(0.02);

     javax.swing.JPanel pc = new ChartPanel(jfc);
     tabbedPane.add("Inventory On Order Chart", pc);
