   ...
   implements MouseListener, MouseMotionListener
{
   ...
   private static final Logger log =
            LoggerFactory.getLogger(CloseableTabbedPane.class);
  ...
  /**
   * The normal close icon.
   */
  private Icon normalCloseIcon = null;

  /**
   * The close icon when the mouse is over.
   */
  private Icon hooverCloseIcon = null;

  /**
   * The close icon when the mouse is pressed.
   */
  ...
  private void init(int horizontalTextPosition)
  {
    ...
    setCloseIcons(Utils.getCloseIconNorm(),
                  Utils.getCloseIconHover(),
                  Utils.getCloseIconPressed());
   ...
   * @param normal the normal close icon
   * @param hoover the close icon when the mouse is over
   * @param pressed the close icon when the mouse is pressed
   */
  ...
  /**
   * If the title does not exist, it will add a new tab, if it doe exist
   * if will update the existing tab with the component.
   * @param title - Title of the component (tab display title)
   * @param component - Component to put in the tab.
   */
  public void addOrUpdateTab(String title, Component component)
  {
    int index = -1;
    if (null != title);
    {
  ...
  public void addTab(String title, Component component, Icon extraIcon)
  {
    ...
    super.addTab(title, doPaintCloseIcon ? new CloseTabIcon(extraIcon) : null,
                                                                    component);
  ...
  /**
   * Sets the close icon at <code>index</code>.
  ...
  /**
   * Adds an <code>CloseableTabbedPaneListener</code> to the tabbed pane.
  ...
  /**
   * Removes an <code>CloseableTabbedPaneListener</code> from the tabbed pane.
    ...
    implements Icon
  {
     ...
     * @param fileIcon the additional file icon, if there is one set
     */
    ...
    public void paintIcon(Component c, Graphics g, int x, int y)
    {
      ...
      // FWF: for some reason, the tab does not always return isClosable property
      // so the tab close icon does not get drawn.  This only happens when you
      // opened enough tabs that you have more than one row of tabs.  And even
      // then it only happens when you click on the two tabs that are on the
      // bottom row closest to the left side of the Frame.  Always drawing it
